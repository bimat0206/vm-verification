...
func getFunctionName(skip int) string {
	...
	return os.Getenv("COMPONENT_NAME")
}
...
func FormatProductMappings(positionMap map[string]promptutils.ProductInfo) []promptutils.ProductMapping {
	if positionMap == nil {
		return []promptutils.ProductMapping{}
	}
	
	mappings := make([]promptutils.ProductMapping, 0, len(positionMap))
	for position, info := range positionMap {
		mappings = append(mappings, promptutils.ProductMapping{
			Position:    position,
...
func FormatMachineStructure(ms *promptutils.MachineStructure) string {
	if ms == nil {
