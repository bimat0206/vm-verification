...
func (s *InitService) validateRequest(request InitRequest) error {
    ...
    if request.VerificationType != VerificationTypeLayoutVsChecking && 
       request.VerificationType != VerificationTypePreviousVsCurrent {
    ...
    if request.VerificationType == VerificationTypeLayoutVsChecking {
        ...
        if request.LayoutId == 0 {
            return fmt.Errorf("%w: layoutId required for %s", ErrMissingRequiredField, VerificationTypeLayoutVsChecking)
        }
...
func (s *InitService) verifyResources(ctx context.Context, request InitRequest) (*ResourceValidation, error) {
    // Create validation result
    resourceValidation := &ResourceValidation{
        ValidationTimestamp: FormatISO8601(),
    }
    ...
    go func() {
        ...
        resourceValidation.ReferenceImageExists = true
        errChan <- nil
    }()
    ...
    go func() {
        ...
        resourceValidation.CheckingImageExists = true
        errChan <- nil
    }()
...
func (s *InitService) createHistoricalContext(previous *VerificationContext) *HistoricalContext {
