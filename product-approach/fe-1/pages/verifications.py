...
def app(api_client):
    ...
    limit_default = st.session_state.get('verification_last_limit', 20)
...
def render_verification_card(v, idx, status, verification_id, result_key, is_expanded, api_client):
    ...
    if is_expanded:
        ...
        with llm_img_cols[0]:
            ...
            if ref_s3_uri:
                ...
                if ref_bucket_name and ref_key:
                    # Determine bucket type more reliably
                    ref_bucket_type = determine_bucket_type(ref_bucket_name)

                    if ref_bucket_type:
                        # Show loading state
                        with st.spinner("Loading reference image..."):
                            try:
                                if api_client:
                                    url_response = api_client.get_image_url(ref_key, ref_bucket_type)
                                    image_url = url_response.get('presignedUrl') or url_response.get('url')
                                    if image_url:
                                        st.image(
                                            image_url,
                                            use_column_width=True,
                                            caption=f"📁 {ref_key.split('/')[-1]}"
                                        )
                                        image_urls_to_display['reference'] = image_url
                                        # Show image details
                                        st.markdown(f"<small style='color: #A0A0A0;'>🗂️ Path: {ref_key}</small>", unsafe_allow_html=True)
                                    else:
                                        st.warning("⚠️ Image URL not available")
                                else:
                                    st.error("❌ API client not available")
                            except Exception as e_img:
                                logger.error(f"Error getting reference image URL for {ref_key}: {e_img}")
                                st.error(f"❌ Failed to load image: {str(e_img)}")
                    else:
                        st.warning(f"⚠️ Unknown bucket type: {ref_bucket_name}")
                else:
        ...
        with llm_img_cols[1]:
            ...
            if chk_s3_uri:
                ...
                if chk_bucket_name and chk_key:
                    ...
                    if chk_bucket_type:
                        # Show loading state
                        with st.spinner("Loading checking image..."):
                            try:
                                if api_client:
                                    ...
                                    if image_url:
                                        st.image(
                                            image_url,
                                            use_column_width=True,
                                            caption=f"📁 {chk_key.split('/')[-1]}"
                                        )
                                        image_urls_to_display['checking'] = image_url
                                        # Show image details
                        ...
                        st.warning(f"⚠️ Unknown bucket type: {chk_bucket_name}")
                else:
                    st.error("❌ Invalid S3 URI format")
            else:
        ...
        llm_analysis = extract_llm_analysis(v, detailed_verification)
        if llm_analysis:
        ...
        # Show additional verification details if available from detailed fetch
        if detailed_verification and detailed_verification != v:
            st.markdown("**📋 Additional Details:**")
            show_additional_verification_details(detailed_verification, v)

        # Raw data section (collapsible)
        raw_data_available = False
        raw_result_data = v.get('result') or (detailed_verification.get('result') if detailed_verification else None)
        raw_summary_data = v.get('verificationSummary') or (detailed_verification.get('verificationSummary') if detailed_verification else None)
