...
# Deploy script for API Verifications List Lambda Function
# This script builds the Docker image and pushes it to ECR

set -e

# Configuration
ECR_REPO="879654127886.dkr.ecr.us-east-1.amazonaws.com/kootoro-dev-ecr-api-verifications-list-f6d3xl"
FUNCTION_NAME="kootoro-dev-lambda-api-verifications-list-f6d3xl"
...
get_ecr_repository() {
    ...
    # Get the repository name that contains 'api-verifications-list'
    REPO_NAME=$(aws ecr describe-repositories --region $AWS_REGION --query "repositories[?contains(repositoryName, 'api-verifications-list')].repositoryName" --output text 2>/dev/null | head -1)

    if [ -z "$REPO_NAME" ]; then
        log_error "ECR repository not found. Please ensure Terraform has been applied and the ECR repository exists."
        log_info "Expected repository name pattern: *api-verifications-list*"
...
build_and_test() {
    ...
    GOWORK=off VERIFICATION_TABLE=test-table go test -v

    # Build binary
    log_info "Building binary..."
    GOWORK=off go build -o api-verifications-list *.go
...
update_lambda() {
    ...
    LAMBDA_FUNCTION_NAME=$(aws lambda list-functions --query "Functions[?contains(FunctionName, 'api-verifications-list')].FunctionName" --output text 2>/dev/null | head -1)
...
deploy() {
    log_info "Starting deployment of API Verifications List Lambda Function..."
    ...
    log_info "The API Verifications List Lambda function is now deployed and ready to use."
    log_info "Endpoint: GET /api/verifications"
}
...
go_build() {
    ...
    GOWORK=off go build -o api-verifications-list *.go
    log_success "Binary built: api-verifications-list"
}

go_clean() {
    log_info "Cleaning up..."
    rm -f api-verifications-list
...
go_run() {
    ...
    log_info "  export VERIFICATION_TABLE=your-verification-table"
