locals {
  ...
  ecr_repositories = {
    ...
    api_verifications_status = {
      name                 = lower(join("-", compact([local.name_prefix, "ecr", "api-verifications-status", local.name_suffix])))
      image_tag_mutability = "MUTABLE"
      scan_on_push         = true
      force_delete         = false
      encryption_type      = "AES256"
    ...
    api_images_upload = {
      name                 = lower(join("-", compact([local.name_prefix, "ecr", "api-images-upload", local.name_suffix])))
      image_tag_mutability = "MUTABLE"
      scan_on_push         = true
      force_delete         = false
      encryption_type      = "AES256"
    ...
    api_images_upload_render = {
      name                 = lower(join("-", compact([local.name_prefix, "ecr", "api-images-upload-render", local.name_suffix])))
      image_tag_mutability = "MUTABLE"
      scan_on_push         = true
      force_delete         = false
      encryption_type      = "AES256"
    ...
    react_frontend = {
      name                 = lower(join("-", compact([local.name_prefix, "ecr", "react-frontend", local.name_suffix])))
      image_tag_mutability = "MUTABLE"
      scan_on_push         = true
      force_delete         = false
      encryption_type      = "AES256"
  ...
  lambda_functions = {
    ...
    api_verifications_status = {
      name        = lower(join("-", compact([local.name_prefix, "lambda", "api-verifications-status", local.name_suffix]))),
      description = "API endpoint for retrieving verification status details",
      memory_size = 512,
      timeout     = 30,
      environment_variables = {
        DYNAMODB_VERIFICATION_TABLE      = local.dynamodb_tables.verification_results
    ...
    api_images_upload = {
      name        = lower(join("-", compact([local.name_prefix, "lambda", "api-images-upload", local.name_suffix]))),
      description = "Upload files to S3 buckets via REST API (legacy)",
      memory_size = 256,
      timeout     = 30,
      environment_variables = {
        REFERENCE_BUCKET = local.s3_buckets.reference
        ...
        LOG_LEVEL        = "INFO"
        DYNAMODB_LAYOUT_TABLE = local.dynamodb_tables.layout_metadata
        JSON_RENDER_PATH      = "s3://${local.s3_buckets.reference}/raw/"
      }
    },
    api_images_upload_render = {
      name        = lower(join("-", compact([local.name_prefix, "lambda", "api-images-upload-render", local.name_suffix]))),
      description = "Upload files to S3 buckets via REST API and automatically render JSON layout files to images",
      memory_size = 1024,
      timeout     = 300,
      environment_variables = {
        REFERENCE_BUCKET      = local.s3_buckets.reference
