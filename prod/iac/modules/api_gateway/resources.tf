...
resource "aws_api_gateway_rest_api" "api" {
  ...
  # Configure binary media types to handle file uploads properly
  # This is essential for image uploads to prevent binary data corruption
  binary_media_types = [
    "multipart/form-data",
    "image/*",
...
resource "aws_api_gateway_resource" "verifications_status" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  parent_id   = aws_api_gateway_resource.verifications.id
  path_part   = "status"
}

# /api/verifications/status/{verificationId}
resource "aws_api_gateway_resource" "verifications_status_id" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  parent_id   = aws_api_gateway_resource.verifications_status.id
  path_part   = "{verificationId}"
}
