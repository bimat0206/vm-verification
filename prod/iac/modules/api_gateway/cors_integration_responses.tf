...
# 8. Verification Status - GET /api/verifications/status
resource "aws_api_gateway_integration_response" "verifications_status_get" {
  count = var.cors_enabled ? 1 : 0
  ...
  status_code = aws_api_gateway_method_response.verifications_status_get.status_code
