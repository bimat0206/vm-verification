...
# Deploy script for API Images Upload Render Lambda Function
# This script builds the Docker image and pushes it to ECR

set -e

# Configuration
FUNCTION_NAME="kootoro-dev-lambda-api-images-upload-render-f6d3xl"
ECR_REPO="879654127886.dkr.ecr.us-east-1.amazonaws.com/kootoro-dev-ecr-api-images-upload-render-f6d3xl"
IMAGE_TAG="latest"
...
get_ecr_repository() {
    ...
    # Get the repository name that contains 'api-images-upload-render'
    REPO_NAME=$(aws ecr describe-repositories --region $AWS_REGION --query "repositories[?contains(repositoryName, 'api-images-upload-render')].repositoryName" --output text 2>/dev/null | tr '\t' '\n' | head -1)

    if [ -z "$REPO_NAME" ]; then
        log_error "ECR repository not found. Please ensure Terraform has been applied and the ECR repository exists."
        log_info "Expected repository name pattern: *api-images-upload-render*"
...
build_and_test() {
    ...
    REFERENCE_BUCKET=test-ref CHECKING_BUCKET=test-check go test -v > /dev/null 2>&1
...
update_lambda() {
    ...
    LAMBDA_FUNCTION_NAME=$(aws lambda list-functions --query "Functions[?contains(FunctionName, 'api-images-upload-render')].FunctionName" --output text 2>/dev/null | head -1)
    ...
    cat > test_payload.json << 'EOF'
{
  ...
  "body": "{\"layoutId\":12345,\"subLayoutList\":[{\"trayList\":[{\"trayCode\":\"A\",\"trayNo\":1,\"slotList\":[{\"vmLayoutSlotId\":1,\"productId\":100,\"productTemplateId\":200,\"maxQuantity\":10,\"slotNo\":1,\"status\":1,\"position\":\"A1\",\"slotIndexCode\":1,\"cellNumber\":1,\"productTemplateName\":\"Test Product\",\"productTemplateImage\":\"https://via.placeholder.com/100x100\"}]}]}]}"
}
EOF
...
deploy() {
    log_info "Starting deployment of API Images Upload Render Lambda Function..."
    ...
    log_info "The API Images Upload Render Lambda function is now deployed and ready to use."
    log_info "Endpoint: POST /api/images/upload"
    log_info "Features: File upload + automatic JSON layout rendering"
    log_info "Parameters: bucketType (reference|checking), fileName, path (optional)"
}
...
go_run() {
    ...
    log_info "  export JSON_RENDER_PATH=s3://your-bucket/raw/"
    log_info "  export DYNAMODB_LAYOUT_TABLE=VendingMachineLayoutMetadata"
...
case "${1:-deploy}" in
    ...
    "help"|"-h"|"--help")
        ...
        echo ""
        echo "Features:"
        echo "  - File upload to S3 buckets"
        echo "  - Automatic JSON layout rendering"
        echo "  - Environment-driven configuration"
        echo "  - Cross-bucket rendering support"
        ;;
    *)
