...
# 7. Get Verification Status - GET /api/verifications/status
resource "aws_api_gateway_method" "verifications_status_get" {
  rest_api_id      = aws_api_gateway_rest_api.api.id
  resource_id      = aws_api_gateway_resource.verifications_status.id
  http_method      = "GET"
  authorization    = var.use_api_key ? "NONE" : "NONE"
  api_key_required = var.use_api_key
  ...
  request_validator_id = aws_api_gateway_request_validator.params_only_validator.id
  request_parameters = {
    "method.request.querystring.verificationId" = false
...
resource "aws_api_gateway_method_response" "verifications_status_get" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  resource_id = aws_api_gateway_resource.verifications_status.id
  http_method = aws_api_gateway_method.verifications_status_get.http_method
  status_code = "200"
...
# OPTIONS method for verifications_status
resource "aws_api_gateway_method" "verifications_status_options" {
  rest_api_id   = aws_api_gateway_rest_api.api.id
  resource_id   = aws_api_gateway_resource.verifications_status.id
  http_method   = "OPTIONS"
  authorization = "NONE"
...
resource "aws_api_gateway_integration" "verifications_status_options" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  resource_id = aws_api_gateway_resource.verifications_status.id
  http_method = aws_api_gateway_method.verifications_status_options.http_method
  type        = "MOCK"
  request_templates = {
    "application/json" = "{ \"statusCode\": 200 }"
  }
...
resource "aws_api_gateway_method_response" "verifications_status_options" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  resource_id = aws_api_gateway_resource.verifications_status.id
  http_method = aws_api_gateway_method.verifications_status_options.http_method
  status_code = "200"
...
resource "aws_api_gateway_integration_response" "verifications_status_options" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  resource_id = aws_api_gateway_resource.verifications_status.id
  http_method = aws_api_gateway_method.verifications_status_options.http_method
  status_code = aws_api_gateway_method_response.verifications_status_options.status_code
...
resource "aws_api_gateway_integration" "verifications_status_get" {
  rest_api_id             = aws_api_gateway_rest_api.api.id
  resource_id             = aws_api_gateway_resource.verifications_status.id
  http_method             = aws_api_gateway_method.verifications_status_get.http_method
  integration_http_method = "POST"
...
# 10. Upload Image - POST /api/images/upload
resource "aws_api_gateway_method" "image_upload_post" {
  rest_api_id      = aws_api_gateway_rest_api.api.id
...
# OPTIONS method for image_upload
resource "aws_api_gateway_method" "image_upload_options" {
  rest_api_id   = aws_api_gateway_rest_api.api.id
  resource_id   = aws_api_gateway_resource.image_upload.id
  http_method   = "OPTIONS"
  authorization = "NONE"
...
resource "aws_api_gateway_integration" "image_upload_options" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  resource_id = aws_api_gateway_resource.image_upload.id
  http_method = aws_api_gateway_method.image_upload_options.http_method
  type        = "MOCK"
  request_templates = {
    "application/json" = "{ \"statusCode\": 200 }"
  }
...
resource "aws_api_gateway_method_response" "image_upload_options" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  resource_id = aws_api_gateway_resource.image_upload.id
  http_method = aws_api_gateway_method.image_upload_options.http_method
  status_code = "200"
...
resource "aws_api_gateway_integration_response" "image_upload_options" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  resource_id = aws_api_gateway_resource.image_upload.id
  http_method = aws_api_gateway_method.image_upload_options.http_method
  status_code = aws_api_gateway_method_response.image_upload_options.status_code
...
resource "aws_api_gateway_integration" "image_upload_post" {
  rest_api_id             = aws_api_gateway_rest_api.api.id
  resource_id             = aws_api_gateway_resource.image_upload.id
  http_method             = aws_api_gateway_method.image_upload_post.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = "arn:aws:apigateway:${var.region}:lambda:path/2015-03-31/functions/${var.lambda_function_arns["api_images_upload_render"]}/invocations"

  # Configure content handling for binary data
  # CONVERT_TO_BINARY ensures that binary content is properly base64 encoded
  content_handling = "CONVERT_TO_BINARY"
}

resource "aws_api_gateway_method_response" "image_upload_post" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  resource_id = aws_api_gateway_resource.image_upload.id
  http_method = aws_api_gateway_method.image_upload_post.http_method
  status_code = "200"
...
resource "aws_api_gateway_integration_response" "image_upload_post" {
  count       = var.cors_enabled ? 1 : 0
  rest_api_id = aws_api_gateway_rest_api.api.id
  resource_id = aws_api_gateway_resource.image_upload.id
  http_method = aws_api_gateway_method.image_upload_post.http_method
  status_code = aws_api_gateway_method_response.image_upload_post.status_code
