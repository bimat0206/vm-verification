...
resource "aws_api_gateway_deployment" "deployment" {
  ...
  triggers = {
    redeployment = sha1(jsonencode([
      ...
      aws_api_gateway_method.image_upload_post.id,
      aws_api_gateway_integration.image_upload_post.id,
      aws_api_gateway_method_response.image_upload_post.id,
      var.cors_enabled ? aws_api_gateway_integration_response.image_upload_post[0].id : "",

      aws_api_gateway_method.image_upload_options.id,
      aws_api_gateway_integration.image_upload_options.id,
      aws_api_gateway_method_response.image_upload_options.id,
      aws_api_gateway_integration_response.image_upload_options.id,
    ]))
  }
  ...
  depends_on = [
    ...
    aws_api_gateway_integration.image_upload_post,
    ...
    aws_api_gateway_method_response.image_upload_post,
    ...
    aws_api_gateway_integration.image_upload_options,
    aws_api_gateway_method_response.image_upload_options,
    aws_api_gateway_integration_response.image_upload_options
  ]
}
...
resource "aws_api_gateway_stage" "stage" {
  ...
  variables = {
    ...
    verification_status_lambda       = var.lambda_function_names["api_verifications_status"]
    ...
    image_upload_lambda              = var.lambda_function_names["api_images_upload_render"]
  }
