...
# Deploy script for API Images View Lambda Function
# This script builds the Docker image and pushes it to ECR

set -e

# Configuration
FUNCTION_NAME="kootoro-dev-lambda-api-images-view-f6d3xl"
ECR_REPO="879654127886.dkr.ecr.us-east-1.amazonaws.com/kootoro-dev-ecr-api-images-view-f6d3xl"
IMAGE_TAG="latest"
...
get_ecr_repository() {
    ...
    # Get the repository name that contains 'api-images-view'
    REPO_NAME=$(aws ecr describe-repositories --region $AWS_REGION --query "repositories[?contains(repositoryName, 'api-images-view')].repositoryName" --output text 2>/dev/null | head -1)

    if [ -z "$REPO_NAME" ]; then
        log_error "ECR repository not found. Please ensure Terraform has been applied and the ECR repository exists."
        log_info "Expected repository name pattern: *api-images-view*"
...
update_lambda() {
    ...
    LAMBDA_FUNCTION_NAME=$(aws lambda list-functions --query "Functions[?contains(FunctionName, 'api-images-view')].FunctionName" --output text 2>/dev/null | head -1)
...
{
  "httpMethod": "GET",
  "path": "/api/images/test-image.jpg/view",
...
deploy() {
    log_info "Starting deployment of API Images View Lambda Function..."
    ...
    log_info "The API Images View Lambda function is now deployed and ready to use."
    log_info "Endpoint: GET /api/images/{key}/view"
    log_info "Query Parameters: bucketType=reference|checking"
}
...
go_build() {
    ...
    go build -o api-images-view *.go
    log_success "Binary built: api-images-view"
}

go_clean() {
    log_info "Cleaning up..."
    rm -f api-images-view
