...
# Deploy script for API Images Upload Lambda Function
# This script builds the Docker image and pushes it to ECR

set -e

# Configuration
FUNCTION_NAME="kootoro-dev-lambda-api-images-upload-f6d3xl"
ECR_REPO="879654127886.dkr.ecr.us-east-1.amazonaws.com/kootoro-dev-ecr-api-images-upload-f6d3xl"
IMAGE_TAG="latest"
...
get_ecr_repository() {
    ...
    # Get the repository name that contains 'api-images-upload'
    REPO_NAME=$(aws ecr describe-repositories --region $AWS_REGION --query "repositories[?contains(repositoryName, 'api-images-upload')].repositoryName" --output text 2>/dev/null | head -1)

    if [ -z "$REPO_NAME" ]; then
        log_error "ECR repository not found. Please ensure Terraform has been applied and the ECR repository exists."
        log_info "Expected repository name pattern: *api-images-upload*"
...
build_and_test() {
    ...
    go build -o api-images-upload *.go
...
update_lambda() {
    ...
    LAMBDA_FUNCTION_NAME=$(aws lambda list-functions --query "Functions[?contains(FunctionName, 'api-images-upload')].FunctionName" --output text 2>/dev/null | head -1)
...
deploy() {
    log_info "Starting deployment of API Images Upload Lambda Function..."
    ...
    log_info "The API Images Upload Lambda function is now deployed and ready to use."
...
go_build() {
    ...
    go build -o api-images-upload *.go
    log_success "Binary built: api-images-upload"
}

go_clean() {
    log_info "Cleaning up..."
    rm -f api-images-upload
