...
func (h *Handler) HandleRequest(ctx context.Context, input *internal.StepFunctionInput) (*internal.StepFunctionOutput, error) {
	...
	if input.S3References == nil || len(input.S3References) == 0 {
		return nil, wferrors.NewValidationError("S3References is required", nil)
	}
	
	// Map S3References to StateReferences format
	s3References := input.MapS3References()
	
	// Validate that we have proper references after mapping
	if s3References == nil {
		return nil, wferrors.NewValidationError("Failed to map S3References to StateReferences", nil)
	}
	
	// Check if VerificationId is provided and consistent
	verificationId := s3References.VerificationId
	if verificationId == "" {
		// Try to get from direct input if not in references
		verificationId = input.VerificationId
		if verificationId == "" {
			return nil, wferrors.NewValidationError("VerificationId is required", nil)
		}
		// Set it in the references for consistency
		s3References.VerificationId = verificationId
	}
	...
	log.Info("Starting ExecuteTurn1 with state references", map[string]interface{}{
		"verificationId": verificationId,
		"datePartition":  s3References.DatePartition,
	})

	// Step 1: Validate state references
	if err := h.validator.ValidateStateReferences(s3References); err != nil {
	...
	state, err := h.stateLoader.LoadWorkflowState(ctx, s3References)
	if err != nil {
