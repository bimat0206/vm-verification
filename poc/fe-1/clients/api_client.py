...
class APIClient:
    ...
    def upload_file(self, file_content, filename, bucket_type='reference', path=''):
        """
        Upload a file using the API endpoint instead of direct S3 upload.
        
        Args:
            file_content: The file content (bytes or file-like object)
            filename: Name of the file
            bucket_type: 'reference' or 'checking'
        ...
        base = self.base_url.rstrip('/')
        endpoint = f"{base}/api/images/upload"
        ...
        try:
            logger.info(f"Uploading file {filename} to {bucket_type} bucket via API")
            
            # Prepare multipart form data
            files = {'file': (filename, file_content)}
            
            # Make the request without Content-Type header to let requests set it for multipart
