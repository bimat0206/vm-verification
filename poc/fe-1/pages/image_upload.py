...
def upload_via_api(api_client, uploaded_file, bucket_type, path, filename):
    """Upload file using the API endpoint instead of direct S3 upload"""
    try:
        ...
        if response.get('success', False):
            ...
            if files:
                ...
                s3_url = f"s3://{uploaded_file_info['bucket']}/{uploaded_file_info['key']}"
                return True, s3_url, uploaded_file_info
            else:
                return False, "No file information returned from API", None
        else:
        ...
        if is_valid:
            ...
            if st.button(f"Upload to {title} Bucket", key=f"{bucket_type}_upload_btn"):
                try:
                    ...
                    # Determine upload path and filename for API call
                    if bucket_type == "reference":
                        upload_path = current_selected_path if current_selected_path else ""
                        upload_filename = stored_filename
                    else:
                        # For checking bucket, extract path and filename from final_path
                        if "/" in final_path:
                            upload_path = "/".join(final_path.split("/")[:-1])
                            upload_filename = final_path.split("/")[-1]
                        else:
                            upload_path = ""
                            upload_filename = final_path

                    # Show upload progress
                    with st.spinner(f"Uploading via API..."):
                        success, result, file_info = upload_via_api(
                            api_client, uploaded_file, bucket_type, upload_path, upload_filename
                        )
                    ...
                    if success:
                        ...
                        if file_info:
                            st.write(f"**Bucket:** {file_info['bucket']}")
                            st.write(f"**Key:** {file_info['key']}")
                            st.write(f"**Size:** {file_info['size']:,} bytes")
                            st.write(f"**Content Type:** {file_info['contentType']}")
                        ...
                        st.session_state[f"{bucket_type}_uploads"].append(upload_record)
...
def app(api_client):
    ...
    # Note about API upload
    st.info("📝 **Note:** This tool uploads files via the API endpoint, which handles S3 storage automatically. No direct AWS credentials required.")
